...
export const modelModalities = ["Dispensa sem justa causa pela empresa", "Dispensa por justa causa", "Pedido de demissão", "Término de contrato de trabalho", "Abandono de emprego", "Contrato de Experiência"] as const;
export default function ProcessModelsModal({ documents, busy, editable, error, onClose, onUpload, onOpen }: {
  ...
  return createPortal(<div className="models-modal-backdrop" onClick={(event) => { if (event.target === event.currentTarget && !busy) onClose(); }}>
    <div className="models-modal" ref={panel} role="dialog" aria-modal="true" aria-labelledby="models-modal-title" onKeyDown={(event) => {
      ...
      <header><span className="models-modal-icon"><Archive size={25} /></span><div><h2 id="models-modal-title">Modelos utilizados</h2><p>Organize seus modelos por tipo de desligamento.</p></div><button className="icon-button" type="button" disabled={busy} onClick={onClose} aria-label="Fechar modelos"><X size={20} /></button></header>
      <div className="models-modal-body">
        ...
        <div className="models-drawers">{modelModalities.map((modality, index) => {
          const oldFolders: Record<string, string[]> = {
            "Dispensa sem justa causa pela empresa": ["Aviso prévio indenizado"],
            "Pedido de demissão": ["Aviso prévio do empregado"],
            "Contrato de Experiência": ["Desativação rápida"],
          };
          const files = documents.filter((document) => [modality, ...(oldFolders[modality] || [])].some((folder) => document.folderPath.replace(/^Modelos utilizados\s*\/\s*/i, "").toLocaleLowerCase("pt-BR") === folder.toLocaleLowerCase("pt-BR")));
      ...
      <footer><span>PDF, Word e imagens · Vários arquivos por tipo de desligamento</span><button type="button" className="btn btn-secondary" disabled={busy} onClick={onClose}>Fechar</button></footer>
    </div>
  </div>, document.body);
}

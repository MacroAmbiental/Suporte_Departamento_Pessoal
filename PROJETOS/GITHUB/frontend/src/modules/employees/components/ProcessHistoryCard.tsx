...
export default function ProcessHistoryCard() {
  ...
  const rows = employeeProcessHistory.filter((item) => `${item.employee.name} ${item.employee.cpf} ${item.company} ${item.modality}`.toLocaleLowerCase("pt-BR").includes(search.toLocaleLowerCase("pt-BR"))).sort((a, b) => b.completedAt.localeCompare(a.completedAt));
  const pages = Math.max(1, Math.ceil(rows.length / 10));
  const current = Math.min(page, pages);
  const offset = (current - 1) * 10;
  return <section className="employee-process-list">
    <header className="process-history-heading"><div><h2><History size={22} /> Histórico de processos</h2><p className="muted">{employeeProcessHistory.length} processos arquivados. Informações preservadas no encerramento.</p></div><input aria-label="Pesquisar histórico de processos" placeholder="Pesquisar no histórico..." value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} /></header>
    <div className="employee-process-table-wrap"><table className="data-table"><thead><tr><th>Funcionário</th><th>Empresa</th><th>Modalidade</th><th>Inicio do processo</th><th>Conclusão</th><th>Observações</th></tr></thead><tbody>
      {rows.slice(offset, offset + 10).map((item) => <tr key={item.id} onDoubleClick={(event) => { event.stopPropagation(); setSelectedId(item.id); }}><td><button className="process-open-button" type="button" onClick={() => setSelectedId(item.id)}>{item.employee.name}</button></td><td>{item.company}</td><td>{item.modality}</td><td>{item.enteredAt ? formatDate(item.enteredAt.slice(0, 10)) : "Não registrado"}</td><td>{formatDate(item.completedAt.slice(0, 10))}</td><td style={{ whiteSpace: "pre-wrap" }}>{item.notes || "—"}</td></tr>)}

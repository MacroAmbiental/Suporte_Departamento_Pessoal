...
export interface Vacation {
  id: string;
  employeeId: string;
  startDate: string; // yyyy-mm-dd — início das férias (opcional se houver motivo)
  endDate: string; // yyyy-mm-dd — retorno (opcional se houver motivo)
  days: number;
  acquisitionYear?: number; // período aquisitivo (ex.: 2024)
  acquisitionStart?: string; // início do período aquisitivo (opcional)
  soldStartDate?: string; // início do período vendido (abono)
  soldEndDate?: string; // fim do período vendido (abono)
  daysSold?: number; // dias vendidos (abono pecuniário) — derivado do período
  soldNote?: string; // legado: descrição de quais dias foram vendidos
  motivo?: string; // obrigatório quando não há início/fim
  notes?: string;
  createdAt: string;
  updatedAt: string;
}
...
export async function saveVacation(
  ...
  // Firestore rejeita valores undefined — remove antes de gravar.
...
/** Diferença em meses inteiros entre duas datas ISO (yyyy-mm-dd). */
...
/**
 * Data de referência do período aquisitivo em aberto:
 * retorno das férias mais recentes (endDate) ou, se nunca houve, a admissão.
 */
export function vacationReferenceDate(
  ...
  // Considera apenas férias JÁ CONCLUÍDAS (retorno anterior a hoje) para resetar
  // o período aquisitivo. Férias em andamento/programadas não resetam ainda.
...
/** Soma anos/meses/dias a uma data ISO (yyyy-mm-dd) e devolve ISO. */
...
export interface AcquisitionPeriod {
  index: number;
  year: number; // ano de início do período aquisitivo
  startISO: string; // início (aniversário da admissão)
  endISO: string; // último dia do período aquisitivo (12 meses - 1 dia)
  limitISO: string; // data limite de gozo (fim + 12 meses — regra CLT)
  complete: boolean; // período aquisitivo já encerrado
}

/**
 * Gera todos os períodos aquisitivos a partir do aniversário da admissão até
 * hoje (mais recente primeiro). Ex.: admitido 15/03/2020 → 2020/21, 2021/22, ...
 */
...
/** Dias de direito conforme art. 130 da CLT a partir das faltas injustificadas. */
...
/** Classifica o funcionário conforme os meses desde a data de referência. */
export function vacationBucket(months: number): VacationBucket {
  if (months >= 24) return "critico";
  if (months >= 17) return "limite"; // regra da empresa: 1 ano e 5 meses
...
/** Possui férias com início no futuro (programadas). */
...
/** Verifica se uma data ISO está dentro do período de férias. */

...
function employeeProcessBadge(employee: Employee, date: string) {
  ...
  if (label && (!end || date <= end)) {
    const reduction = fields.noticeReductionApplies === "true" && fields.noticeReduction === "hours" && noticeWorkAdjustment(employee, date) === "hours" ? " · redução de 2h/dia" : "";
    return `${label}${reduction}`;
  }
  if (fields.noticeStartDate && end && date >= fields.noticeStartDate && date < end) return "Aviso prévio";
  if (!label && (fields.scheduledDeactivationDate || fields.deactivationEffectiveDate)) return terminationModes.quick;
...
function createDisplayRecord(
  ...
  const status = suspensionDay
    ? "absence_confirmed"
    : noticeAdjustment === "leave" && (!existing?.status || isFullDayAbsenceStatus(existing.status))
      ? "day_off"
      :
    existing?.status ||
...
export default function Timekeeping() {
  ...
  // Férias registradas (módulo de Férias) — usadas para marcar status "Férias".
  const [vacationsAll, setVacationsAll] = useState<Vacation[]>([]);
  useEffect(() => {
    void loadVacations()
      .then(setVacationsAll)
      .catch(() => setVacationsAll([]));
  }, []);
  const vacationEmployeeIds = useMemo(() => {
    const byEmp = new Map<string, Vacation[]>();
    vacationsAll.forEach((v) => {
      const list = byEmp.get(v.employeeId) || [];
      list.push(v);
      byEmp.set(v.employeeId, list);
    });
    const set = new Set<string>();
    byEmp.forEach((list, empId) => {
      if (isOnVacation(filters.date, list)) set.add(empId);
    });
    return set;
  }, [vacationsAll, filters.date]);
  ...
  const displayRecordByEmployeeId = useMemo(() => {
    ...
    employees.forEach((employee) => {
      ...
      const rec = createDisplayRecord(
        employee,
        filters.date,
        withSec,
        calculationSettings,
      );
      // Sincroniza com o módulo de Férias: se o funcionário está de férias no
      // dia e não há status/batida explícita, marca o status como "Férias".
      if (
        vacationEmployeeIds.has(employee.id) &&
        (rec.status === "absence_pending" || rec.status === "day_off")
      ) {
        rec.status = "vacation";
      }
      map.set(employee.id, rec);
    });
    return map;
  }, [
    calculationSettings,
  ...
  function renderCell(
    ...
    if (column.type === "system") {
      const processBadge = column.systemField === "employee" ? employeeProcessBadge(employee, filters.date) : null;
      return (
        <td
          key={column.key}
        ...
        >
          {processBadge ? (
            <span className="timekeeping-employee-name">
              ...
              <span className="benefit-cell-value">{systemValue(employee, record, column)}</span>
            </span>
          ) : (
            <span className="benefit-cell-value">{systemValue(employee, record, column)}</span>
          )}
